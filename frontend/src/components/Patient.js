...
export default function PatientDashboard() {
  ...
  const fetchPatientProfile = async () => {
    try {
      ...
      const response = await fetch(
        "http://hms-anas-backend.vercel.app/api/patient/profile",
        {
          headers: {
  ...
  const fetchDoctors = async () => {
    try {
      ...
      const response = await fetch(
        "http://hms-anas-backend.vercel.app/api/doctor/all",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const fetchAvailableSlots = async (doctorId, date) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `http://hms-anas-backend.vercel.app/api/patient/available-slots?doctorId=${doctorId}&date=${date}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const response = await fetch(
        "http://hms-anas-backend.vercel.app/api/patient/appointments",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const fetchCareTeam = async () => {
    try {
      ...
      const response = await fetch(
        "http://hms-anas-backend.vercel.app/api/patient/care-team",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const fetchPrescriptions = async () => {
    try {
      ...
      const response = await fetch(
        "http://hms-anas-backend.vercel.app/api/patient/prescriptions",
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
          ...
          <CardContent>
            ...
            {appointments.length > 0 ? (
              <p className="text-xs text-gray-500">
                ...
                {appointments[0].doctorId.lastName} at {appointments[0].time}
              </p>
            ) : (
  ...
  const renderAppointmentBooking = () => {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(
          "http://hms-anas-backend.vercel.app/api/patient/book-appointment",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
      ...
      <nav className="bg-blue-700 text-white p-4">
        <ul className="flex space-x-4 justify-center">
          ...
          <li>
            <Button
              variant={
                activeTab === "Appointment Booking" ? "outline" : "ghost"
              }
              className={`hover:bg-white hover:text-blue-600 ${
